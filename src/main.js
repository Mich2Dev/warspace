...
function createLaser(position, velocity, colorHex) {
  const geometry = new THREE.CylinderGeometry(2, 2, 80, 8);
...
function fireLaser() {
  ...
  let forward = new THREE.Vector3(0, 0, -1).applyQuaternion(spaceship.mesh.quaternion);
  ...
  if (lockedTarget && !lockedTarget.isDead) {
    ...
    // Auto-Aim (Asistencia de apuntado)
    let bestTarget = null;
    let bestDot = 0.92;
    for (const id in remotePlayers) {
      const rp = remotePlayers[id];
  ...
  // Laser starting position (from the nose of the ship)
  const startPos = spaceship.mesh.position.clone().add(forward.clone().multiplyScalar(15));
  
  // Laser velocity (very fast)
  const laserSpeed = 8000;
...
document.addEventListener('click', (e) => {
  if (!isMapMode) {
    if (document.pointerLockElement === document.body) {
      fireLaser(); // Fire weapon if already locked
    } else {
      document.body.requestPointerLock();
    }
  } else {
...
document.addEventListener('keydown', (e) => {
  if (e.code === 'Space' && !isMapMode && document.pointerLockElement === document.body) {
    fireLaser();
  }
});
...
function updateNavComputer() {
  ...
  planets.forEach((p, index) => {
    ...
    li.innerText = `Planeta Sector ${index + 1}`;
